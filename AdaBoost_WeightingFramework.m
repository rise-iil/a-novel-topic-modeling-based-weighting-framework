function [prediction,score] = AdaBoost_WeightingFramework (TRAIN,TEST,WeakLearn,prob_doc)
% This function implements the AdaBoost_WeightingFramework Algorithm. 
...
W = zeros(1,m);
prob_doc1 = prob_doc;
% Min- Max Normalize the Positive class instance weights
minval = min(prob_doc(1:pos_size));
maxval = max(prob_doc(1:pos_size));
prob_doc1(1:pos_size) = (prob_doc1(1:pos_size)-minval)./(maxval-minval);

% Min- Max normalize the Negative class instance weights
minval = min(prob_doc(pos_size+1:end));
maxval = max(prob_doc(pos_size+1:end));
prob_doc1(pos_size+1:end) = (prob_doc1(pos_size+1:end)-minval)./(maxval-minval);
...
while t <= T
    ...
    % Updating weight
    for i = 1:m
        W(t+1,i) = W(t,i)*exp(-AlphaT(t)*pred(i)*TRAIN(i,end));
    end
