function [prediction,score] = AdaCost_WeightingFramework(TRAIN,TEST,WeakLearn,prob_doc)
...
while t <= T
    ...
    r = sum(W(t,:)'.*pred.*TRAIN(:,end).*Beta(pred,TRAIN(:,end),costs));
    AlphaT(t) = 0.5*log((1+r)/(1-r));
    ...
    % Updating weight
    for i = 1:m
        W(t+1,i) = W(t,i)*exp(-AlphaT(t)*pred(i)*TRAIN(i,end)*Beta(pred(i),TRAIN(i,end), costs(i)));
    end
...
function beta = Beta(yi, hi, cost)
    % beta_ = 0.5c + 0.5
    % beta+ = -0.5c + 0.5
    beta = 0.5 - (yi.*hi).*0.5.*cost;
end
